...
/**
 *
 * @author Cesars Pinedo
 * @version 0.1
 */
public class Botonredondo extends AbstractButton implements MouseListener {

    /**
     * size behavior, if true ignores xDiameter and yDiameter and uses width and
     * heigth for size or from layout behavior if false will force the dimension
     * to xDiameter and yDiameter overriden layout behavior
     */
     ...
     */
    private Color colorin = new Color(192, 57, 43);
    private Color colorfin = new Color(41, 128, 185);
    private int wUsed;
    private int hUsed;
    private int xIni;
    private int yIni;
    ...
    public Botonredondo(Color ini, Color hover) {
        autoSize = true;
        jobInitialize();
        this.colorin = ini;
        this.colorfin = hover;
     ...
     */
    public Botonredondo(int xDiameter, int yDiameter) {
     ...
     * @param borderFocusColor Color to paint the border when JOvalbutton has
     * the focus
     */
    public Botonredondo(int xDiameter, int yDiameter, Color borderColor, Color borderFocusColor) {
        this.xDiameter = xDiameter;
        this.yDiameter = yDiameter;
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (autoSize) {
            ...
            if (lmg instanceof GridBagLayout || lmg instanceof FlowLayout) {
                chBounds = false;
            }
            if (chBounds && wUsed > this.getWidth()) {
                wUsed = this.getWidth();
            }
            if (chBounds && hUsed > this.getHeight()) {
                hUsed = this.getHeight();
            }
            if (chBounds && wUsed < this.getWidth()) {
                xIni = ((this.getWidth() - wUsed) / 2);
            }
            if (chBounds && hUsed < this.getHeight()) {
                yIni = ((this.getHeight() - hUsed) / 2);
            }
        }
    ...
    @Override
    public void mouseEntered(MouseEvent e) {
        this.setBackground(colorin);
    }

    @Override
    public void mouseExited(MouseEvent e) {
        this.setBackground(colorfin);
    }
}
